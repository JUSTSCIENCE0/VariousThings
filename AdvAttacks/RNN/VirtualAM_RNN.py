...
def get_prediction(x):
    ...
    for i in range(result.shape[0]):
        result[i][np.argmax(result,axis=1)[i]] = 1.0
    result = np.where(result>1, 0.9, result)
    return result
...
for iteration in range(30):
    ...
    attack = VirtualAdversarialMethod(classifier, max_iter = 1)
